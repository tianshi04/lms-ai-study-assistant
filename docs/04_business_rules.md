...
* **BR_ACCESS_004 (Phân loại Trả phí Cá nhân - Mua lẻ vs Thuê bao Coursera Plus & Quy định Giá):**
  ...
  * *Quy định Giá Mua lẻ (Course Pricing Authority):* Mức giá (`price`) và đơn vị tiền tệ (`currency`) của từng khóa học do **Giảng viên sở hữu (`owner_id`)** hoặc **Chủ sở hữu Tổ chức (`Organization Owner`)** thiết lập trực tiếp trong giao diện Course Builder. **Super Admin** quản lý khung giá mặc định (Default Price Tier) và chính sách khuyến mãi toàn sàn. Khi học viên thanh toán (`PurchaseCourse`), Backend truy vấn giá niêm yết trực tiếp từ `CourseModel` để khởi tạo hóa đơn thanh toán, tuyệt đối không tin tưởng giá gửi lên từ Client.
