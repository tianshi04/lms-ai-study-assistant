...
export default function BecomeAnInstructorPage() {
  ...
  const activeApp = existingApp || (submitSuccess ? { status: InstructorApplicationStatus.PENDING_REVIEW } : null);
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-10 shadow-xl border border-slate-200 dark:border-slate-800 text-center relative overflow-hidden">
          ...
          <p className="mt-3 text-slate-600 dark:text-slate-300 max-w-2xl mx-auto text-base sm:text-lg">
            Trở thành Giảng viên trên nền tảng Coursera AI, chia sẻ tri thức chuyên môn đến hàng ngàn học viên và khẳng định thương hiệu cá nhân của bạn.
          </p>
        </div>
        ...
        {isLoadingApp ? (
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-12 text-center border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Đang kiểm tra hồ sơ đăng ký của bạn...</p>
          </div>
        ) : isInstructor ? (
          ...
          <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-3xl p-8 text-center space-y-4 shadow-sm">
            ...
            <p className="text-slate-600 dark:text-slate-300 max-w-lg mx-auto">
              Bạn có thể truy cập ngay Cổng Giảng viên (Instructor Portal) để bắt đầu biên soạn và đăng tải các khóa học mới.
            </p>
            <div className="pt-2">
              <Link
          ...
          <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 rounded-3xl p-8 text-center space-y-4 shadow-sm">
            ...
            <p className="text-slate-600 dark:text-slate-300 max-w-lg mx-auto">
              Bạn cần có tài khoản cá nhân trên hệ thống để gửi đơn xin cấp quyền Giảng viên và theo dõi kết quả thẩm định.
            </p>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-10 shadow-xl border border-slate-200 dark:border-slate-800 space-y-6">
            ...
            <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-200 text-sm flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5">Ban Quản trị nền tảng (Super Admin) đang tiến hành thẩm định thông tin năng lực chuyên môn và video demo. Kết quả sẽ được tự động cập nhật ngay trên trang này.</p>
              </div>
            </div>

            {existingApp && (
              <div className="space-y-4 pt-2 text-sm">
                <h3 className="font-bold text-slate-900 dark:text-white text-base">Thông tin đã nộp:</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                  <div>
                    <span className="text-xs text-slate-500 dark:text-slate-400 block font-medium">Chức danh đăng ký:</span>
                    <span className="font-bold text-slate-800 dark:text-slate-200">{existingApp.title}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-500 dark:text-slate-400 block font-medium">Thời gian nộp:</span>
                    <span className="font-medium text-slate-800 dark:text-slate-200">{existingApp.createdAt ? new Date(existingApp.createdAt).toLocaleString("vi-VN") : "Gần đây"}</span>
                  </div>
                  {existingApp.linkedinUrl && (
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 block font-medium">LinkedIn/Portfolio:</span>
                      <a href={existingApp.linkedinUrl} target="_blank" rel="noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline font-medium truncate block">
                        {existingApp.linkedinUrl}
                      </a>
                    </div>
                  )}
                  {existingApp.cvUrl && (
                    <div>
                      <span className="text-xs text-slate-500 dark:text-slate-400 block font-medium">File CV (.pdf):</span>
                      <a href={existingApp.cvUrl} target="_blank" rel="noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline font-medium truncate block">
                        {existingApp.cvUrl}
                      </a>
                    </div>
                  )}
                ...
                <div>
                  <span className="text-xs text-slate-500 dark:text-slate-400 block font-medium mb-1">Tiểu sử năng lực:</span>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-10 shadow-xl border border-slate-200 dark:border-slate-800 space-y-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Đơn nộp ngày: {existingApp?.createdAt ? new Date(existingApp.createdAt).toLocaleDateString("vi-VN") : "Trước"}
                </p>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-800 text-rose-800 dark:text-rose-200 text-sm space-y-2">
              ...
              <p><span className="font-semibold">Lý do từ chối:</span> {existingApp?.rejectionReason || "Hồ sơ chưa đáp ứng đầy đủ tiêu chuẩn chuyên môn."}</p>
            </div>

            <p className="text-xs text-slate-500 dark:text-slate-400">
              Bạn có thể chỉnh sửa lại các thông tin bằng cách bổ sung thêm kinh nghiệm, làm rõ thành tựu hoặc cập nhật CV/Video demo mới để nộp lại đơn.
            </p>
          ...
          /* Application Form */
          <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-10 shadow-xl border border-slate-200 dark:border-slate-800 space-y-6">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-4 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <svg className="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span>{isReapplying ? "Soạn Lại Đơn Đăng Ký Mới" : "Thông tin Hồ sơ Thẩm định"}</span>
              </h2>
              {isReapplying && (
                <button
                  type="button"
            ...
            {isReapplying && (
              <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-200 text-xs">
                ...
                <span>Vui lòng bổ sung hoặc điều chỉnh thông tin dựa trên lý do từ chối trước đó ({existingApp?.rejectionReason}).</span>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Chức danh này sẽ hiển thị bên cạnh tên của bạn trên các khóa học sau khi được phê duyệt.
              </p>
            </div>
