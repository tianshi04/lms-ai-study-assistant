  ...
  if (!partner) {
    return (
      <div className="max-w-md mx-auto my-20 p-8 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-center shadow-sm">
        ...
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Không tìm thấy Đối tác</h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm mb-6">
          Đối tác phát hành với đường dẫn &quot;{slug}&quot; không tồn tại hoặc đã dừng hoạt động.
        </p>
        <Button onClick={() => router.push("/")} className="bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl">
          Quay lại trang chủ
        </Button>
      </div>
    );
      ...
      <div className="relative w-full bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 text-white">
        {partner.bannerUrl ? (
          <div className="relative h-64 sm:h-80 w-full overflow-hidden">
            <Image src={partner.bannerUrl} alt={partner.name} fill className="object-cover opacity-40" unoptimized />
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-end space-y-4 sm:space-y-0 sm:space-x-6">
              ...
              <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-2xl bg-white dark:bg-slate-900 p-3 shadow-xl border-4 border-white dark:border-slate-800 flex items-center justify-center overflow-hidden shrink-0">
                {partner.logoUrl ? (
                  <Image src={partner.logoUrl} alt={partner.name} width={120} height={120} className="object-contain max-h-full" unoptimized />
                ) : (
                  <span className="font-extrabold text-3xl text-blue-600">{partner.name.substring(0, 2).toUpperCase()}</span>
                )}
              </div>
              ...
              <div>
                ...
                <h1 className="text-3xl sm:text-4xl font-extrabold text-white mt-1 tracking-tight">{partner.name}</h1>
                {partner.websiteUrl && (
                  <a
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 shadow-sm">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-4">Về {partner.name}</h2>
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Các khóa học do {partner.name} phát hành</h2>
                  <p className="text-xs text-slate-500 mt-1">Học viên hoàn thành khóa học sẽ nhận chứng chỉ điện tử được xác thực bởi {partner.name}.</p>
                </div>
                <span className="text-sm font-semibold text-blue-600 dark:text-blue-400">{partnerCourses.length} Khóa học</span>
              </div>
              ...
              {partnerCourses.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center border border-slate-200 dark:border-slate-800">
                  ...
                  <p className="text-slate-600 dark:text-slate-400 font-medium">Hiện chưa có khóa học nào được phát hành bởi đối tác này.</p>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                        <div className="flex items-center space-x-1 text-amber-500 text-xs font-semibold">
                          ...
                          <span className="text-slate-400 font-normal">({course.reviewCount || 0})</span>
                        </div>
          ...
          <div className="space-y-6">
            ...
            {(partner.signerName || partner.signerTitle) && (
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                <h3 className="text-sm font-bold uppercase text-slate-400 tracking-wider mb-4">Đại diện Phát hành</h3>
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <p className="font-bold text-slate-900 dark:text-white text-base">{partner.signerName}</p>
                ...
                {partner.signatureImageUrl && (
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 text-center">
                    <p className="text-xs text-slate-400 mb-2">Mẫu chữ ký điện tử đã được xác minh:</p>
                    <div className="inline-block p-2 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <Image src={partner.signatureImageUrl} alt="Chữ ký đại diện" width={120} height={48} className="h-12 object-contain" unoptimized />
                    </div>
                  </div>
                )}
              </div>
            ...
            {partner.allowedDomains && partner.allowedDomains.length > 0 && (
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                <h3 className="text-sm font-bold uppercase text-slate-400 tracking-wider mb-3">Tên miền Cấp Chứng chỉ</h3>
                <p className="text-xs text-slate-500 mb-3">Chỉ các chứng chỉ phát hành từ hệ thống thuộc danh sách tên miền dưới đây mới có giá trị pháp lý:</p>
                <div className="flex flex-wrap gap-2">
                  {partner.allowedDomains.map((domain, i) => (
                    <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-mono text-xs rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 p-6 rounded-2xl border border-blue-200 dark:border-blue-800/50">
              <div className="flex items-center space-x-3 mb-2">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white text-sm">Tương thích OpenBadges v2.0 / v3.0</h3>
              </div>
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                Chứng chỉ được cấp bởi {partner.name} chứa chữ ký số mã hóa PKI công khai, tương thích chuẩn toàn cầu OpenBadges và có thể chia sẻ lên LinkedIn, CV điện tử.
              </p>
            </div>
          </div>
        </div>
