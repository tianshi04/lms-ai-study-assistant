          ...
          >
            ...
            <span className="ml-2 text-xs py-0.5 px-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
              {tab.id === "ALL"
                ? courses.length
                : tab.id === "IN_PROGRESS"
                  ? courses.filter((c) => c.status === "IN_PROGRESS" || c.status === "NOT_STARTED")
                      .length
      ...
      {loading ? (
        ...
        >
          {filteredCourses.map((course) => (
            <div
              key={course.courseId}
              className="group bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col"
            >
