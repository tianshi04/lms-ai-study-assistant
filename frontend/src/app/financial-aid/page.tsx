    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="flex items-center gap-3 text-slate-600 dark:text-slate-400">
          ...
          <span className="text-sm font-medium">{t("financialAid.loadingData")}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto px-4 py-12 space-y-8">
      ...
      <div>
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 text-xs font-bold uppercase tracking-wider mb-3">
          {t("financialAid.programTag")}
        </div>
        <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">
          {t("financialAid.pageTitle")}
        </h1>
        <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
          {t("financialAid.pageDesc")}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-3">
        <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400">
          {t("financialAid.selectCourseLabel")}
        </label>
        <select
        ...
        {selectedCourse && (
          <p className="text-xs text-slate-500 dark:text-slate-400 italic">
            {t("financialAid.issuingPartner")}: <strong className="text-slate-700 dark:text-slate-300">{selectedCourse.partnerName}</strong>
          </p>
        )}
      </div>
      ...
      {existingApp && !isReApplying ? (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-xl space-y-6">
          <div className="flex items-center justify-between pb-6 border-b border-slate-200 dark:border-slate-800">
            <div>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">
                {t("financialAid.appCode")}: #{existingApp.id}
              </span>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{t("financialAid.appStatusTitle")}</h2>
            </div>
          ...
          <div className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
            {existingApp.status === "PENDING" && (
              <>
                <p>{t("financialAid.pendingDesc")}</p>
                <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 flex items-center justify-between">
                  <span className="font-medium text-slate-700 dark:text-slate-300">{t("financialAid.estimatedResponse")}</span>
                  <span className="font-bold text-blue-600 dark:text-blue-400">{existingApp.reviewDeadlineDaysLeft} {t("financialAid.daysLeft")}</span>
                </div>
              </>
            )}
            ...
            {existingApp.status === "APPROVED" && (
              <div className="p-5 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-800 dark:text-emerald-300 space-y-2">
                <p className="font-bold text-base flex items-center gap-2">
                  ...
                  {t("financialAid.approvedTitle")}
                </p>
                <p className="text-sm">
                  {t("financialAid.approvedDesc")}
                </p>
              </div>
            )}
            ...
            {existingApp.status === "REJECTED" && (
              <div className="p-5 rounded-2xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-800 dark:text-rose-300 space-y-3">
                <p className="font-bold text-base flex items-center gap-2">
                  ...
                  {t("financialAid.rejectedTitle")}
                </p>
                <p className="text-sm">
                  {t("financialAid.rejectedDesc")}
                </p>
              </div>
            )}
            ...
            <div className="space-y-2 pt-2">
              <span className="text-xs font-bold uppercase tracking-wider text-slate-500">{t("financialAid.submittedEssay")}</span>
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            {existingApp.status === "REJECTED" && (
              ...
              >
                ...
                <span>{t("financialAid.reapplyBtn")}</span>
              </button>
            )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={`text-xs font-bold font-mono px-2.5 py-1 rounded-md ${
                  ...
                  {wordCount} / 150 {t("financialAid.essayWordCount")}
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">
                {t("financialAid.essayHelpText")}
              </p>
              <textarea
                rows={10}
                value={essay}
                onChange={(e) => setEssay(e.target.value)}
                placeholder={t("financialAid.essayPlaceholder")}
            ...
            <div className="p-4 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-xs text-amber-800 dark:text-amber-300 space-y-2">
              <p className="font-bold flex items-center gap-1.5">
                ...
                {t("financialAid.integrityPledgeTitle")}
              </p>
              <p>{t("financialAid.integrityPledgeText")}</p>
            </div>
            ...
            >
              {submitting ? t("financialAid.submittingApp") : t("financialAid.submitAppBtn")}
            </button>
          </form>
        </div>
      )}
