...
export default function AdminPartnersPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="max-w-md mx-auto my-16 p-8 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-2xl text-center">
        ...
        <p className="text-slate-600 dark:text-slate-400 text-sm">Bạn cần quyền Super Admin để truy cập trang quản trị đối tác.</p>
  ...
  const handleAddPartnerAdmin = () => {
    ...
    if (allowedDomains.length > 0 && !allowedDomains.includes(emailDomain)) {
      setAdminErrorMsg(`Email domain (${emailDomain}) không thuộc danh sách Tên miền được phép: ${allowedDomains.join(", ")}`);
      return;
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-6 border-b border-slate-200 dark:border-slate-800 gap-4">
        <div>
          <div className="flex items-center space-x-2 text-sm text-slate-500 mb-1">
            <button onClick={() => router.push("/admin/dashboard")} className="hover:text-blue-600 transition-colors">
              Trang quản trị
            </button>
            <span>/</span>
            <span className="text-slate-800 dark:text-slate-200 font-medium">Đối tác phát hành</span>
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Quản lý Đối tác Phát hành</h1>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">
            Quản lý danh sách các trường đại học, viện nghiên cứu và doanh nghiệp phát hành chứng chỉ số.
          </p>
        </div>
        <Button onClick={handleOpenCreate} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl px-5 py-2.5 shadow-sm">
      ...
      <div className="mt-8 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        {partners.length === 0 ? (
          <div className="p-12 text-center text-slate-500">
            ...
            <p className="text-lg font-medium text-slate-700 dark:text-slate-300">Chưa có đối tác nào được tạo</p>
            <p className="text-sm text-slate-500 mt-1">Bấm nút &quot;Thêm đối tác mới&quot; để thiết lập đối tác phát hành chứng chỉ đầu tiên.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {partners.map((partner) => (
                  <tr key={partner.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 p-1 border border-slate-200 dark:border-slate-700 flex items-center justify-center overflow-hidden shrink-0">
                          {partner.logoUrl ? (
                            <Image src={partner.logoUrl} alt={partner.name} width={36} height={36} className="object-contain max-h-full" unoptimized />
                          ) : (
                            <span className="font-bold text-blue-600 text-sm">{partner.name.substring(0, 2).toUpperCase()}</span>
                          )}
                        </div>
                        <div>
                          <p className="font-semibold text-slate-900 dark:text-white">{partner.name}</p>
                          {partner.websiteUrl && (
                            <a href={partner.websiteUrl} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">
                              {partner.websiteUrl}
                            </a>
                          )}
                        </div>
                      ...
                      {partner.allowedDomains && partner.allowedDomains.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {partner.allowedDomains.map((domain, i) => (
                            <span key={i} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                              {domain}
                            </span>
                          ))}
                        </div>
                    ...
                    <td className="py-4 px-6">
                      <p className="font-medium text-slate-800 dark:text-slate-200">{partner.signerName || "—"}</p>
            ...
            <div className="space-y-2 max-h-36 overflow-y-auto pr-1">
              {currentPartnerAdmins.length === 0 ? (
                ...
                currentPartnerAdmins.map((admin) => (
                  <div key={admin.id} className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{admin.name}</span>
