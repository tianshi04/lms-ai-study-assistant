...
export default function AdminCategoriesPage() {
  ...
  if (profileLoading) return <div className="p-8 text-center">{"Đang tải..."}</div>;
  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-12 flex-1">
        <div className="mb-8 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">{"Danh mục quản trị"}</h1>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <CategoryList title={"Danh sách chủ đề"} items={subjects} type="SUBJECT" handleDelete={handleDelete} noCategoriesText={"Chưa có danh mục"} deleteText={"Xoá"} />
          <CategoryList title={"Danh sách cấp độ"} items={levels} type="LEVEL" handleDelete={handleDelete} noCategoriesText={"Chưa có danh mục"} deleteText={"Xoá"} />
        </div>
      </main>
  );
}
