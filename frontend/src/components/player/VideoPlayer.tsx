...
export function VideoPlayer({
  ...
  if (activeItem.type === 3 || activeItem.type === 4) {
    return (
      <div className="w-full h-full overflow-y-auto p-6 bg-card">
        <GradedQuizRunner
        ...
        <div className="w-full h-full max-h-[60vh] 2xl:max-h-[680px] relative flex items-center justify-center bg-muted/50 border border-border/60 rounded-xl overflow-hidden transition-colors duration-200 shadow-sm">
          ...
          {activeQuiz && (
            <div className="absolute inset-0 bg-background/80 backdrop-blur-md z-30 flex items-center justify-center p-6 animate-in fade-in duration-200">
              <div className="bg-card text-card-foreground border border-border rounded-2xl p-6 max-w-lg w-full shadow-2xl space-y-4">
                ...
                <div className="pt-2 flex justify-end gap-3">
                  {!quizSubmitted ? (
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-primary hover:bg-primary-hover disabled:opacity-50 text-primary-foreground font-bold text-xs shadow-sm transition-all cursor-pointer"
                    >
                      {"Kiểm Tra Đáp Án"}
                    </button>
                  ) : (
                    <button
                      ...
                      className="px-5 py-2.5 rounded-xl bg-success hover:bg-success-hover text-success-foreground font-bold text-xs shadow-sm transition-all flex items-center gap-2 cursor-pointer"
                    >
                      {"Tiếp Tục Xem Video"}
