  ...
  (
    ...
    useImperativeHandle(ref, () => ({
      get currentTime() {
        ...
        if (internalYoutubePlayerRef.current)
          return internalYoutubePlayerRef.current.getCurrentTime() || 0;
        return 0;
      },
      get duration() {
        if (internalVideoRef.current) return internalVideoRef.current.duration;
        if (internalYoutubePlayerRef.current)
          return internalYoutubePlayerRef.current.getDuration() || 0;
        return 0;
      },
      pause() {
    ...
    if (activeItem.type === 1 && activeItem.videoUrl) {
      ...
      return (
        <div ref={containerRef} className="w-full h-full relative flex items-center justify-center bg-slate-100 dark:bg-black transition-colors duration-200 group">
          {youtubeId ? (
            <YouTube
              key={activeItem.id}
              videoId={youtubeId}
            ...
            <>
              <video
                ...
                className="w-full h-full object-contain shadow-2xl rounded-lg border border-slate-300 dark:border-slate-800"
              />
          ...
          {/* Custom Floating Controls Wrapper */}
          <div className="absolute bottom-4 right-4 z-30 flex flex-col items-end gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
            {/* Speed Menu Dropdown */}
            {isSpeedMenuOpen && (
              <div className="bg-black/80 backdrop-blur-xl rounded-2xl p-2 shadow-2xl border border-white/10 flex flex-col gap-1 w-36 animate-in fade-in slide-in-from-bottom-2 origin-bottom-right">
                {[0.5, 0.75, 1, 1.25, 1.5, 2].map((rate) => (
                  <button
                    ...
                    className={`px-3 py-2 text-sm font-bold rounded-xl transition-all text-left ${
                      playbackRate === rate
            ...
            {/* Glass Pill */}
            <div className="flex items-center gap-1 bg-black/60 backdrop-blur-md border border-white/10 p-1 rounded-full shadow-2xl">
              {/* PiP Button (Only for native video) */}
              {!youtubeId && isPipSupported && (
                <button
                  onClick={handleTogglePip}
                  title="Picture-in-Picture"
                  className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/20 text-white transition-all"
                >
              ...
              <button
                ...
                className="px-4 h-10 rounded-full hover:bg-white/20 text-white transition-all flex items-center gap-1.5 font-bold text-sm"
                title="Tốc độ phát"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
