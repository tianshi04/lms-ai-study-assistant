...
### Integration with Base UI (@base-ui/react)

This pattern is used extensively by [Base UI](https://base-ui.com/), which automatically applies state data attributes to its primitives:

```tsx
import { Dialog } from "@base-ui/react/dialog";

<Dialog.Root>
  <Dialog.Trigger className="data-[state=open]:bg-accent" />
  <Dialog.Portal>
    {/* Base UI automatically adds data-open, data-closed, data-starting-style, data-ending-style */}
    <Dialog.Backdrop className="data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 transition-opacity" />
    <Dialog.Popup className="data-[open]:animate-in data-[closed]:animate-out" />
  </Dialog.Portal>
</Dialog.Root>;
```

Other data attributes Base UI provides include:

- `data-open` / `data-closed` - open/closed modal or popup state
- `data-starting-style` / `data-ending-style` - CSS transition entry/exit states
- `data-side` - top/right/bottom/left (for positioned elements)
- `data-align` - start/center/end (for positioned elements)
- `data-disabled` - present when element is disabled
- `data-checked` - present when checkbox/toggle is checked
