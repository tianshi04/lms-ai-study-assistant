...
- **3-Layer Security Protocol (Hybrid PBAC & Multi-Tenant SQL Scope Pushdown)**:
  ...
  1. **Layer 1 (API Method Policy & Context Injection)**: Every ConnectRPC method in `.proto` MUST declare custom option `(auth.v1.policy)` (`AUTH_POLICY_PUBLIC`, `AUTH_POLICY_AUTHENTICATED`). `AuthInterceptor` & `AuthPolicyRegistry` validate JWT and inject `CurrentUserContext` (`src.shared.auth`). Resource creation/mutation operations receive `organization_id` directly from the Protobuf request payload.
  2. **Layer 2 (Database Query / SQL Scope Pushdown)**: Repositories querying organization-scoped entities MUST execute `apply_organization_scope(stmt, Model, get_current_user())` (`src.shared.infrastructure.scopes`), filtering queries by user role context and `INTERNAL_SYSTEM_ORG_ID`. In-memory organization filtering is strictly prohibited.
  3. **Layer 3 (Domain Resource Ownership & Centralized ReBAC Matrix)**: All mutating Application Use Cases **MUST** explicitly execute authorization checks using central guards (`enforce_organization_permission` or `enforce_course_ownership` from `src.shared.permissions`). No mutating operation may bypass permission checking.
...
- **No Hardcoded Absolute Paths Rule**: Documentation files in `docs/` **MUST NOT** contain hardcoded absolute filesystem paths or environment-specific file URLs (e.g. `file:///...`, `C:\...`). Always use relative repository paths.
